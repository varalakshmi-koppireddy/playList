...
import PlaySongItem from '../PlaySongItem'
...
class PlaySongs extends Component {
  ...
  onDeleteIcon = id => {
    ...
    const filteredPlayItems = tracksList.filter(each => each.id !== id)

    this.setState({tracksList: filteredPlayItems})
  }
  ...
  render() {
    ...
    return (
      <BgContainer>
        ...
        <BottomContainer>
          <SearchDesContainer>
            <ItemText>Songs Playlist</ItemText>
            <SearchContainer>
              <InputSearch
                type="search"
                value={searchInput}
